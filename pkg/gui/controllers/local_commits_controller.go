...
func (self *LocalCommitsController) startInteractiveRebaseWithEdit(
	...
	return self.c.WithWaitingStatus(self.c.Tr.RebasingStatus, func(gocui.Task) error {
		...
		startIdx, endIdx, rangeSelectMode := self.context().GetSelectionRangeAndMode()
		commits := self.c.Model().Commits
		selectionStartSha := commits[startIdx].Sha
		selectionEndSha := commits[endIdx].Sha
		...
		return self.c.Helpers().MergeAndRebase.CheckMergeOrRebaseWithRefreshOptions(
			...
			types.RefreshOptions{Mode: types.BLOCK_UI, Then: func() {
				...
				_, startIndex, ok1 := lo.FindIndexOf(self.c.Model().Commits, func(c *models.Commit) bool {
					return c.Sha == selectionStartSha
				})
				_, endIndex, ok2 := lo.FindIndexOf(self.c.Model().Commits, func(c *models.Commit) bool {
					return c.Sha == selectionEndSha
				})
				if ok1 && ok2 {
					self.context().SetSelectionRangeAndMode(startIndex, endIndex, rangeSelectMode)
				}
			}})
	})
}
