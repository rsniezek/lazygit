...
| `` v `` | Переключить выборку перетаскивания | Toggle drag selection mode. |
...
| `` C `` | Скопировать несколько отобранных коммитов (cherry-pick) | Mark commits as copied from the last copied commit to the selected commit. |
...
| `` <c-t> `` | Open external diff tool (git difftool) |  |
| `` y `` | Скопировать атрибут коммита | Copy commit attribute to clipboard (e.g. hash, URL, diff, message, author). |
| `` <enter> `` | Просмотреть файлы выбранного элемента | View the files modified by the selected item. |
| `` w `` | View worktree options |  |
| `` / `` | Найти |  |
