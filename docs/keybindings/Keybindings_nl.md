...
| `` C `` | Kopieer commit reeks (cherry-pick) | Mark commits as copied from the last copied commit to the selected commit. |
| `` <c-r> `` | Reset cherry-picked (gekopieerde) commits selectie |  |
| `` <c-t> `` | Open external diff tool (git difftool) |  |
| `` y `` | Copy commit attribute to clipboard | Copy commit attribute to clipboard (e.g. hash, URL, diff, message, author). |
| `` <enter> `` | Bekijk gecommite bestanden | View the files modified by the selected item. |
| `` w `` | View worktree options |  |
| `` / `` | Start met zoeken |  |
...
| `` v `` | Toggle drag selecteer | Toggle drag selection mode. |
