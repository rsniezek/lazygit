...
| `` C `` | 複製提交範圍 (揀選) | Mark commits as copied from the last copied commit to the selected commit. |
| `` <c-r> `` | 重設選定的揀選 (複製) 提交 |  |
...
| `` <left> `` | 選擇上一段 |  |
| `` <right> `` | 選擇下一段 |  |
| `` v `` | 切換拖曳選擇 | Toggle drag selection mode. |
| `` V `` | 切換拖曳選擇 |  |
...
| `` <c-t> `` | Open external diff tool (git difftool) |  |
| `` y `` | 複製提交屬性 | Copy commit attribute to clipboard (e.g. hash, URL, diff, message, author). |
| `` <enter> `` | 檢視所選項目的檔案 | View the files modified by the selected item. |
| `` w `` | View worktree options |  |
| `` / `` | 開始搜尋 |  |
