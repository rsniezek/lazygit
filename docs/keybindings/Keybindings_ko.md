...
| `` C `` | 커밋을 범위로 복사 (cherry-pick) | Mark commits as copied from the last copied commit to the selected commit. |
...
| `` r `` | Rename stash |  |
| `` <enter> `` | View selected item's files | View the files modified by the selected item. |
| `` w `` | View worktree options |  |
| `` / `` | Filter the current view by text |  |
...
| `` <left> `` | 이전 hunk를 선택 |  |
| `` <right> `` | 다음 hunk를 선택 |  |
| `` v `` | 드래그 선택 전환 | Toggle drag selection mode. |
| `` V `` | 드래그 선택 전환 |  |
